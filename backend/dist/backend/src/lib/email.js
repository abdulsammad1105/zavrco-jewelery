...
const order_id_1 = require("./order-id");
async function sendOrderConfirmationEmail(order) {
    ...
    if (Array.isArray(order.items)) {
        formattedItems = order.items
            .map((item) => `${item.name || item.title} × ${item.quantity || item.units || 1}`)
            .join('\n');
    }
    ...
    const formattedId = typeof rawId === 'number' ? (0, order_id_1.formatOrderId)(rawId) : (rawId || 'N/A');
    ...
    try {
        const response = await nodejs_1.default.send(serviceId, templateId, templateVars, {
            publicKey,
            privateKey,
        });
